...
var getExchangeRateFromOffers = function getExchangeRateFromOffers(_ref) {
  ...
  return destinationAmount && sendMax ? _promise2.default.reject(new Error('Cannot set both destinationAmount and sendMax')) : Orderbook({ buying: destinationAsset, selling: sourceAsset }).then(function (o) {
    var offers = o.bids;
    if (offers.length === 0) {
...
var getPathSource = function getPathSource(_ref2) {
  ...
  return Paths({
    source: source, destination: destination, destinationAsset: destinationAsset, destinationAmount: destinationAmount
